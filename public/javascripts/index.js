...
function MovePostList ( sortType ) {
  ...
  .then(response => {
    ...
    response.data.forEach((e,i) => {
      if(i<10) { // 임시코드 수정하기
        document.getElementById(`category-post-${i}`).innerText = e.title;
      }
    });
    
  document.getElementById(`main-post-container`).style.display = 'none';
  document.getElementById(`category-post-container`).style.display = 'block';
